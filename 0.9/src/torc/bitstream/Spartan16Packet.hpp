...
/// \file
/// \brief Header for the Spartan16Packet class.

#ifndef TORC_BITSTREAM_SPARTAN16_PACKET_HPP
#define TORC_BITSTREAM_SPARTAN16_PACKET_HPP
...
namespace bitstream { class bitstream_spartan16_packet; }

	/// \brief Bitstream packet constants for Spartan 16 bit class architectures.
	class Spartan16PacketConstants {
	public:
	// enumerations
	...
	class Spartan16Packet : public Spartan16PacketConstants {
		friend class torc::bitstream::bitstream::bitstream_spartan16_packet;
	protected:
	// typedefs
	...
	// constructors
		...
		Spartan16Packet(void) : mHeader(0), mCount(0), mWord(0), mWords(0), mType(EPacketType(0)), 
			mOpcode(eOpcodeNOP), mAddress(0) {}
		/// \brief Full constructor.
		Spartan16Packet(uint16_t inHeader, uint32_t inCount, uint16_t inWord, 
		...
		Spartan16Packet(uint16_t inHeader, uint16_t inWord) : mHeader(inHeader), mCount(1), 
			mWord(inWord), mWords(0), mType(EPacketType(0)), mOpcode(eOpcodeNOP), mAddress(0) {
			initialize();
		}
		/// \brief Header only constructor.
		Spartan16Packet(uint16_t inHeader) : mHeader(inHeader), mCount(0), mWord(0), mWords(0), 
			mType(EPacketType(0)), mOpcode(eOpcodeNOP), mAddress(0) {
			initialize();
		}
		/// \brief Copy constructor.
		Spartan16Packet(const Spartan16Packet& rhs) : mHeader(rhs.mHeader), mCount(rhs.mCount),
	 		mWord(rhs.mWord), mWords(rhs.mWords), mType(EPacketType(0)), mOpcode(eOpcodeNOP), mAddress(0) {
			initialize();
		}
	// functions
		/// \brief Read a packet.
		static Spartan16Packet read(std::istream& inStream) {
			uint32_t count = 0;
			uint16_t count1 = 0; 
			uint16_t count2 = 0; 
			uint16_t header = 0;
			...
			if(header == eSynchronizationDummy || header == eSynchronizationSync0
			  ...
			  || header == eSynchronizationBusWidthSync1 || header == eSynchronizationBusWidthDetect0
				...
				return Spartan16Packet(header, 0, 0, 0);
			}
			...
			return Spartan16Packet(header, count, word, raw_words);
		}
		...
		///		packet.
		static Spartan16Packet makeNullType2Write(uint16_t inAddress) {
			return Spartan16Packet(makeHeader(ePacketType2, eOpcodeWrite, inAddress, 0), 0, 0, 0);
		}
		/// \brief Construct a type 1 write packet.
		static Spartan16Packet makeType1Write(uint16_t inAddress, uint16_t inWord) {
			return Spartan16Packet(makeHeader(ePacketType1, eOpcodeWrite, inAddress, 1), 1, inWord, 0);
		}
		/// \brief Construct a type 1 write packet for two words.
		static Spartan16Packet makeType1Write32(uint16_t inAddress, uint32_t inWord) {
			...
			return Spartan16Packet(makeHeader(ePacketType1, eOpcodeWrite, inAddress, 2), 2, 0, words);
		}
		/// \brief Construct a type 1 write packet for two words.
		static Spartan16Packet makeType1Write(uint16_t inAddress, uint16_t inWord1, uint16_t inWord2) {
			// used to send two packets after header for packets like IDCODE, RDBK_SIGN, FARMAJ, and CRC
			uint16_t* words = new uint16_t[2];
			words[0] = inWord1;
			words[1] = inWord2;
		...
		static Spartan16Packet makeType1Write(uint32_t inCount, uint16_t inAddress, uint16_t* inWords) {
			return Spartan16Packet(makeHeader(ePacketType1, eOpcodeWrite, inAddress, inCount), inCount, 0, inWords);
		}
		/// \brief Construct a type 2 write packet.
		static Spartan16Packet makeType2Write(uint32_t inCount, uint16_t inAddress, uint16_t* inWords) {
			return Spartan16Packet(makeHeader(ePacketType2, eOpcodeWrite, inAddress, 0), inCount, 0, inWords);
		}
...
//		}
    ...
    // const uint16_t* getWords(void) const { return mWords; }
	  // tests
	...
	typedef std::vector<Spartan16Packet> Spartan16PacketVector;

} // namespace bitstream
} // namespace torc

#endif // TORC_BITSTREAM_SPARTAN16_PACKET_HPP
