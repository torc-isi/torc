...
/// \file
/// \brief Source for the Spartan16Packet unit test.
...
BOOST_AUTO_TEST_CASE(bitstream_spartan16_packet) {
	...
	std::vector<Spartan16Packet> packets;
	while(cumulativeWordLength < bitstreamWordLength) {
		Spartan16Packet packet = Spartan16Packet::read(fileStream);
		packets.push_back(packet);
		cumulativeWordLength += packet.getWordSize();
	}

	cumulativeWordLength = 0;
	std::vector<Spartan16Packet>::iterator p = packets.begin();
	std::vector<Spartan16Packet>::iterator e = packets.end();
	while(p < e) {
		Spartan16Packet& packet = *p++;
		cumulativeWordLength += packet.getWordSize();
	}
