...
InterfaceJoinedInfo::Factory::newInterfaceJoinedInfoPtr( 
        const std::list< PortSharedPtr > & inPorts,
        const std::list< PortListSharedPtr > & inPortLists,
        const ViewSharedPtr & inViewPtr,
        const InterfaceJoinedInfoSharedPtr & inParentJoinedInfo ) throw(Error) {
    try
    {
 ...
 */
void 
InterfaceJoinedInfo::setJoinedType(const JoinedType & value) throw() {
    mJoinedType = value;
}
 ...
 */
void
InterfaceJoinedInfo::addPort( const PortSharedPtr & inPort ) throw(Error) {
    if( !inPort )
    {
        return;
    }
...
*/
void
InterfaceJoinedInfo::addPortList( const PortListSharedPtr & inPortList ) throw() {
    if( !inPortList )
    {
        return;
    }
 ...
 */
void
InterfaceJoinedInfo::setPorts( 
        const std::list< PortSharedPtr > & inSource ) throw() {
 ...
 */
void
InterfaceJoinedInfo::setPortLists( 
        const std::list< PortListSharedPtr > & inSource ) throw() {
 ...
 */
void 
InterfaceJoinedInfo::setChildren(
        const std::vector< InterfaceJoinedInfoSharedPtr > & inSource ) throw() {
 ...
 */
bool 
InterfaceJoinedInfo::addChildJoinedInfo(
...
void
InterfaceJoinedInfo::accept(BaseVisitor & visitor) throw(Error) {
    try
    {
 ...
 */
void
InterfaceJoinedInfo::setRelationType(const RelationType & inSource) throw() {
 ...
 */
size_t
InterfaceJoinedInfo::getSize() const throw() {
    size_t size = 0;
    size_t pSize = 0;
    size_t cSize = 0;
    ...
    std::vector< InterfaceJoinedInfoSharedPtr >::iterator joinedInfoIt 
                    = outJoinedInfos.begin();

    std::list< PortSharedPtr > outPorts;
    getPorts( outPorts );
    std::list< PortSharedPtr >::iterator portIt
                    = outPorts.begin();

    std::list< PortListSharedPtr > outPortLists;
    getPortLists( outPortLists );
    std::list< PortListSharedPtr >::iterator portListIt
                    = outPortLists.begin();
...
InterfaceJoinedInfo::InterfaceJoinedInfo()
    : Visitable(),
    SelfReferencing<InterfaceJoinedInfo>(),
    mJoinedType(),
    mRelationType(),
    mJoinedPorts(),
    mJoinedPortLists(),
