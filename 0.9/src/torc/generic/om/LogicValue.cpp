 ...
 */
LogicValueSharedPtr
LogicValue::Factory::newLogicValuePtr( const std::string & inName,
 ...
 */
void 
LogicValue::setAttributes(
        const LogicValueAttributesSharedPtr & inSource) throw() {
    mAttributes = inSource;
}

void
LogicValue::accept(BaseVisitor & inoutVisitor) throw(Error) {
    try
    {
...
LogicValue::LogicValue() 
    ...
    Commentable(),
    Visitable(),
    ParentedObject<SimulationInfo>(),
    PropertyContainer(),
    UserDataContainer(),
    SelfReferencing<LogicValue>(),
    mAttributes() {
}
