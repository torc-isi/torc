 ...
 */
template <typename _Type> class Bundle : public virtual Composite<_Type> {
#ifdef GENOM_SERIALIZATION
...
template <typename _Type> Bundle<_Type>::Bundle() : Composite<_Type>() {}

template <typename _Type> Bundle<_Type>::~Bundle() throw () {}
 ...
 * @return The CompositionType inSource VECTOR is returned
 */
template <typename _Type> CompositionType Bundle<_Type>::getCompositionType() const {
 ...
 * @return Number of bits
 */
template <typename _Type> typename Bundle<_Type>::SizeType Bundle<_Type>::getSize() const {
	size_t size = 0;
	for(typename Bundle<_Type>::List::const_iterator it = mChildren.begin(); it != mChildren.end();
		++it) {
		size += (*it)->getSize();
	}
	return size;
 ...
 * @exception Error The composition is not a container or the child type is not proper
 */
template <typename _Type> void Bundle<_Type>::addChild(const boost::shared_ptr<Type>& child)
	throw (Error) {
	mChildren.push_back(child);
	child->setParentCollection(SelfReferencing<_Type>::getSharedThis());
}
 ...
 * @param[out] outChildren A list of all children for this composition
 */
template <typename _Type> void Bundle<_Type>::getChildren(
	typename Bundle<_Type>::List& outChildren) const throw (Error) {
	outChildren.insert(outChildren.end(), mChildren.begin(), mChildren.end());
	return;
}

template <typename _Type> template <typename _Action> void Bundle<_Type>::applyOnAllChildren(
 ...
 * @exception Error Index dimensions mismatch
 */
template <typename _Type> const typename Bundle<_Type>::Pointer Bundle<_Type>::get(
	const std::vector<typename Bundle<_Type>::SizeType>& indices) const throw (Error) {
	return Pointer();
}

#ifdef GENOM_SERIALIZATION
template <typename _Type>template <class Archive> void Bundle<_Type>::load(Archive& ar,
	unsigned int) {
	ar & boost::serialization::base_object<BaseType>(*this);
	ar & mChildren;
	for_each(mChildren.begin(), mChildren.end(),
		boost::bind(boost::mem_fn(&Composite<_Type>::setParentCollection), _1,
		SelfReferencing<_Type>::getSharedThis()));
}

template <typename _Type>template <class Archive> void Bundle<_Type>::save(Archive& ar,
	unsigned int) const {
	ar & boost::serialization::base_object<BaseType>(*this);
	ar & mChildren;
}
