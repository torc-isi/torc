...
#ifndef TORC_GENERIC_INSTANCEARRAY_HPP
#define TORC_GENERIC_INSTANCEARRAY_HPP
 ...
 *
 * The InstanceArray class represents an array of instances. They can be created using the
 * (instance (array ...) ... ) syntax.
 */
class InstanceArray : public Instance, public Vector<Instance, InstanceArrayMember,
...
public:
	typedef Vector<Instance, InstanceArrayMember, InstanceArrayMember::Factory, false>
		BaseVectorType;
	...
	public:
		 ...
		 **/
		virtual InstanceArraySharedPtr newInstanceArrayPtr(const std::string& inName,
	 ...
	 *
	 */
	template <typename _Action> inline void applyOnAllBits(const _Action& action) throw (Error);

	virtual ~InstanceArray() throw ();

protected:
	virtual void onChildCreate(
 ...
 *
 */
template <typename _Action> inline void InstanceArray::applyOnAllBits(const _Action& action)
	throw (Error) {
	try {
...
#endif // TORC_GENERIC_INSTANCEARRAY_HPP
