...
#ifndef TORC_GENERIC_INTERNALUTILITYFUNCTIONS_HPP
#define TORC_GENERIC_INTERNALUTILITYFUNCTIONS_HPP

namespace torc {
namespace generic {

template <typename _Connectable> void findLeafConnectable(std::vector<std::string>& nestedNames,
...
template <typename _Connectable> void connectNetToElement(const std::vector<size_t>& inIndices,
	const boost::shared_ptr<_Connectable>& inConn, const NetSharedPtr& inNet) throw (Error) {
	try {
		if(inIndices.empty()) {
			//TBD::CHECK WIDTH
...
template <typename _Base, typename _Derived> struct IndexFinder : _Derived::Visitor {
	...
	IndexFinder() : _Derived::Visitor(), mIndices() {
	}
	~IndexFinder() throw () {
	}
...
#endif // TORC_GENERIC_INTERNALUTILITYFUNCTIONS_HPP
