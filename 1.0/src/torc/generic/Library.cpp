...
public:
	void operator()(const CellSharedPtr& inCell) const throw (Error) {
		try {
 ...
 **/
LibrarySharedPtr Library::Factory::newLibraryPtr(const std::string& inName,
	const RootSharedPtr& inRootPtr, const EdifLevel& inEdifLevel, const std::string& inOriginalName)
		throw (Error) {
	try {
...
void Library::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
 ...
 */
void Library::setLevel(const EdifLevel& inSource) {
	mLevel = inSource;
}
 ...
 */
bool Library::findScaleFactor(Unit inUnit, ScaleFactor& outResult) const {
 ...
 *
 */
bool Library::setScaleFactor(Unit inUnit, const ScaleFactor& inScale) {
 ...
 */
void Library::addCell(const CellSharedPtr& inCell) throw (Error) {
	if(!inCell) {
		return;
	}
...
#ifdef GENOM_SERIALIZATION
	std::list<std::string>::iterator it = std::find(mDumpedCells.begin(), mDumpedCells.end(),
		cellName);
 ...
 */
CellSharedPtr Library::findCell(const std::string& inName) throw (Error) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
 ...
 */
void Library::removeCell(const std::string& inName) throw (Error) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
		e.saveContextData("Cell name", inName);
		throw e;
 ...
 */
void Library::getCells(std::vector<CellSharedPtr>& outCells) {
#ifdef GENOM_SERIALIZATION
	restoreAllCells();
#endif //GENOM_SERIALIZATION
	mCellSymTab.getValues(outCells);
}
 ...
 */
void Library::setSimulationInfo(const SimulationInfoSharedPtr& inSource) {
	mSimulationInfo = inSource;
}

Library::Library() : Commentable(), Extern(), Nameable(), Renamable(), Visitable(),
	ParentedObject<Root>(), SelfReferencing<Library>(), UserDataContainer(), StatusContainer(),
	mScaleFactors(), mLevel(), mCellSymTab(), mSimulationInfo() {}
...
#ifdef GENOM_SERIALIZATION
template <class Archive> void Library::save(Archive& ar, unsigned int) const {
...
//                                ParentedObject<Root> >(*this);
	ar & boost::serialization::base_object < SelfReferencing<Library> > (*this);
	ar & mScaleFactors;
	ar & mLevel;
	Cells cells;
	...
	for(; cell != end; ++cell) {
		std::string fileName = getParent()->getDumpRestoreData().getDumpPath() + "/";
		fileName += getName() + "_";
...
template <class Archive> void Library::load(Archive& ar, unsigned int) {
	ar & boost::serialization::base_object < Commentable > (*this);
	ar & boost::serialization::base_object < Extern > (*this);
	ar & boost::serialization::base_object < Nameable > (*this);
	ar & boost::serialization::base_object < Renamable > (*this);
	ar & boost::serialization::base_object < Visitable > (*this);
//    ar & boost::serialization::base_object< ParentedObject<Root> >(*this);
	ar & boost::serialization::base_object < SelfReferencing<Library> > (*this);
	ar & mScaleFactors;
	ar & mLevel;
	ar & mDumpedCells;
...
CellSharedPtr Library::restoreSingleCell(const std::string& inName) throw (Error) {
	CellSharedPtr cell;
	std::list<std::string>::iterator it = std::find(mDumpedCells.begin(), mDumpedCells.end(),
		inName);
	if(it == mDumpedCells.end()) {
...
void Library::restoreAllCells() throw (Error)
try {
	std::vector < std::string > cells;
	cells.insert(cells.end(), mDumpedCells.begin(), mDumpedCells.end());
	for(std::vector<std::string>::iterator cell = cells.begin(); cell != cells.end(); ++cell) {
		try {
...
template void
Library::load<boost::archive::binary_iarchive>(boost::archive::binary_iarchive& ar,
	const unsigned int);

template void
Library::save<boost::archive::binary_oarchive>(boost::archive::binary_oarchive& ar,
...
void dump(const LibrarySharedPtr& inLibrary) throw (Error) {
	...
	try {
		dump(inLibrary, inLibrary->getParent()->getDumpRestoreData());
	} catch(Error& e) {
		e.setCurrentLocation(__FUNCTION__, __FILE__, __LINE__);
		throw;
	}
...
LibrarySharedPtr restore(const std::string& inName, const RootSharedPtr& inParent) throw (Error) {
	...
	try {
		lib = restore(inName, inParent, inParent->getDumpRestoreData());
	} catch(Error& e) {
		e.setCurrentLocation(__FUNCTION__, __FILE__, __LINE__);
		throw;
	}
