...
void ParameterArray::onChildCreate(
	const boost::shared_ptr<BaseVectorType::ChildType>& inCreatedChild) const throw (Error) {
	inCreatedChild->setName(getName());
}

ParameterArray::ParameterArray() : Parameter(), Vector<Parameter, ParameterArrayElement,
 ...
 */
ParameterArraySharedPtr ParameterArray::Factory::newParameterArrayPtr(const std::string& inName,
	const size_t& inSize, const ChildFactorySharedPtr& inFactory, const std::string& inOriginalName)
		throw (Error) {
	try {
...
void ParameterArray::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
...
#ifdef GENOM_SERIALIZATION
template <class Archive> void ParameterArray::serialize(Archive& ar, unsigned int) {
	ar & boost::serialization::base_object< Parameter >(*this);
	ar & boost::serialization::base_object< BaseVectorType >(*this);
}

//TO SATISFY THE LINKER
template void ParameterArray::serialize<boost::archive::binary_oarchive>(
	boost::archive::binary_oarchive& ar, const unsigned int);

template void ParameterArray::serialize<boost::archive::binary_iarchive>(
