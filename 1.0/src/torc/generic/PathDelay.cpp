 ...
 */
PathDelaySharedPtr PathDelay::Factory::newPathDelayPtr(const Value::MiNoMax& inDelay,
	const std::list<EventSharedPtr>& inEvents, const TimingSharedPtr& inTiming) throw (Error) {
	try {
...
void PathDelay::setDelay(const Value::MiNoMax& inSource) {
	mDelay = inSource;
}
 ...
 */
void PathDelay::setEvents(const std::list<EventSharedPtr>& inEvents) {
...
void PathDelay::addEvent(const EventSharedPtr& inEvent) throw (Error) {
	if(!inEvent) {
		return;
	}
...
PathDelay::PathDelay() : SelfReferencing<PathDelay>(), mDelay(), mEvents() {}
