 ...
 * statements. 
 */
class Permutable : public Visitable, public SelfReferencing<Permutable> {
...
public:
	 ...
	 */
	enum PermutableType {
		ePermutableParent = 0, ePermutableChild
	};
	...
	public:
		 ...
		 */
		PermutableSharedPtr virtual newPermutablePtr(const std::vector<PortSharedPtr>& inPorts,
			const ViewSharedPtr& inViewPtr,
			const PermutableSharedPtr& inParentPermutable = PermutableSharedPtr()) throw (Error);
	};
	 ...
	 */
	inline void getChildren(std::vector<PermutableSharedPtr>& outPermutables) const;
	 ...
	 * @return Boolean flag
	 */
	inline const bool getIsNonPermutable() const;
	 ...
	 */
	inline const PermutableType getPermutableType() const;
 ...
 * @param[out] outPorts Vector of ports to be appended to
 */
inline void Permutable::getPorts(std::vector<PortSharedPtr>& outValues) const {
	outValues.insert(outValues.end(), mPorts.begin(), mPorts.end());
}
 ...
 * @param[out] outPermutables Vector of permutable to be appended to
 */
inline void Permutable::getChildren(std::vector<PermutableSharedPtr>& outPermutables) const {
	outPermutables.insert(outPermutables.end(), mChildren.begin(), mChildren.end());
}
 ...
 *
 */
template <typename _Action> inline void Permutable::applyOnAllChildren(const _Action& action)
	throw (Error) {
	try {
 ...
 * @return Boolean flag
 */
inline const bool Permutable::getIsNonPermutable() const {
 ...
 * @return PermutableType
 */
inline const Permutable::PermutableType Permutable::getPermutableType() const {
...
#endif // TORC_GENERIC_PERMUTABLE_HPP
