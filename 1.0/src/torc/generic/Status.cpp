 ...
 */
StatusSharedPtr Status::Factory::newStatusPtr(const StatusContainerSharedPtr& inContainer)
	throw (Error) {
	try {
...
Status::Status() : Commentable(), Visitable(), UserDataContainer(), SelfReferencing<Status>(),
	mWrittens() {}
 ...
 */
void Status::setWrittens(const std::vector<WrittenSharedPtr>& inSource) {
 ...
 */
void Status::addWritten(WrittenSharedPtr& inWritten) throw (Error) {
	mWrittens.push_back(inWritten);
}

void Status::accept(BaseVisitor& visitor) throw (Error) {
	try {
