...
#ifndef TORC_GENERIC_EDIF_DRIVER_HPP
#define TORC_GENERIC_EDIF_DRIVER_HPP
...
public:
	 ...
	 * @return        true if successfully parsed
	 */
	bool parseString(const std::string& inString, const std::string& inStreamName
		= "string stream");
	...
	inline Scanner* getLexer() const;

	//Note: Let the address be returned .. this is used by the parser
	inline std::string& getStreamName();

	inline EdifContextSharedPtr getContext() const;

	inline Error getParserError() const;

	inline bool getIsParserErrorSet() const;
...
inline Scanner* Driver::getLexer() const {
	return mLexer;
}

inline std::string& Driver::getStreamName() {
	return mStreamName;
}

inline EdifContextSharedPtr Driver::getContext() const {
	return mEdifCntx;
}

inline Error Driver::getParserError() const {
	return *mErrorObj;
}
...
#endif // TORC_GENERIC_EDIF_DRIVER_HPP
