...
void PortBundle::setParent(const ViewSharedPtr& inParent) {
	ParentedObject<View>::setParent(inParent);
	applyOnAllChildren(boost::bind(boost::mem_fn(&Port::setParent), _1, getParent()));
}
 ...
 */
Connectable::Connection PortBundle::connect(const NetSharedPtr& inNet) throw (Error) {
	if(!inNet) {
	...
	BundleFlattener<Port, ScalarPort, VectorPort, VectorPortBit, PortBundle> flattener;
	accept(flattener);
 ...
 */
void PortBundle::disconnect(const Connectable::Connection& inConnection) throw (Error) {
 ...
 **/
PortBundleSharedPtr PortBundle::Factory::newPortBundlePtr(const std::string& inName,
	const ViewSharedPtr& inViewPtr, const PortBundleSharedPtr& inParentCollection,
...
void PortBundle::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
...
PortBundle::PortBundle() : Port(), Bundle<Port>() {}
...
#ifdef GENOM_SERIALIZATION
template <class Archive> void PortBundle::serialize(Archive& ar, unsigned int) {
	ar & boost::serialization::base_object< Port >(*this);
	ar & boost::serialization::base_object< Bundle<Port> >(*this);
}

//TO SATISFY THE LINKER
template void PortBundle::serialize<boost::archive::binary_iarchive>(
	boost::archive::binary_iarchive& ar, const unsigned int);

template void PortBundle::serialize<boost::archive::binary_oarchive>(
