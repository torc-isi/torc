...
#ifndef TORC_GENERIC_VECTORPORT_HPP
#define TORC_GENERIC_VECTORPORT_HPP
 ...
 *
 * The VectorPort class represents a port array. Such objects in EDIF are declard using the
 * (port (array ...) ) syntax.
 */
class VectorPort : public Port, public Vector<Port, VectorPortBit, VectorPortBit::Factory, true> {
#ifdef GENOM_SERIALIZATION
...
public:
	...
	public:
		 ...
		 **/
		virtual VectorPortSharedPtr newVectorPortPtr(
			...
			const ChildFactorySharedPtr& inFactory = BaseVectorType::ChildFactorySharedPtr(
				new BaseVectorType::ChildFactory()), const std::string& inOriginalName
		 ...
		 * @return Pointer to created vector port.
		 **/
		virtual VectorPortSharedPtr newVectorPortPtr(const std::string& inName,
			...
			const std::vector<size_t>& inLimits, const PortBundleSharedPtr& inParentCollection
				= PortBundleSharedPtr(), const ChildFactorySharedPtr& inFactory
...
#endif // TORC_GENERIC_VECTORPORT_HPP
