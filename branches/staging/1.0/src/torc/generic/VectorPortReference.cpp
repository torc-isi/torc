...
class PortBinder : public torc::generic::VectorPortBitReference::Visitor {
public:
	void visit(torc::generic::VectorPortBitReference& portBit) throw (torc::generic::Error) {
		torc::generic::PortSharedPtr childPort = mMasterVector->get(portBit.getIndices());
		try {
	...
	PortBinder(const torc::generic::PortSharedPtr& inMasterVector) :
 ...
 **/
VectorPortReferenceSharedPtr VectorPortReference::Factory::newVectorPortReferencePtr(
	const InstanceSharedPtr& inInstancePtr, const PortSharedPtr& inPortPtr, const size_t& inSize,
	const PortBundleReferenceSharedPtr& inParentCollection, const ChildFactorySharedPtr& inFactory)
		throw (Error) {
	try {
		std::vector < size_t > limits;
		limits.push_back(inSize);
		return newVectorPortReferencePtr(inInstancePtr, inPortPtr, limits, inParentCollection,
			inFactory);
	} catch(Error& e) {
		e.setCurrentLocation(__FUNCTION__, __FILE__, __LINE__);
		throw;
...
VectorPortReferenceSharedPtr VectorPortReference::Factory::newVectorPortReferencePtr(
	...
	const std::vector<size_t>& inLimits, const PortBundleReferenceSharedPtr& inParentCollection,
...
void VectorPortReference::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
...
void VectorPortReference::setParent(const InstanceSharedPtr& inParent) {
	ParentedObject<Instance>::setParent(inParent);
	BaseType::List children;
	getCreatedChildren(children);
	std::for_each(children.begin(), children.end(),
		boost::bind(boost::mem_fn(&PortReference::setParent), _1, getParent()));
}
 ...
 */
Connectable::Connection VectorPortReference::connect(const NetSharedPtr& inNet) throw (Error) {
	if(!inNet) {
 ...
 *
 */
void VectorPortReference::disconnect(const Connectable::Connection& inConnection) throw (Error) {
...
void VectorPortReference::getConnectedNets(std::vector<NetSharedPtr>& outNets,
 ...
 */
void VectorPortReference::bindToMasterPort(const PortSharedPtr& inMaster) throw (Error) {
	PortReference::bindToMasterPort(inMaster);

}

void VectorPortReference::onChildCreate(const VectorPortBitReferenceSharedPtr& inCreatedChild) const
	throw (Error) {
...
VectorPortReference::VectorPortReference() : PortReference(), BaseType() {}
