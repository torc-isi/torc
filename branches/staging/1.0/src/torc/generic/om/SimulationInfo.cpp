...
SimulationInfo::Factory::newSimulationInfoPtr( 
        const std::vector< LogicValueSharedPtr > & inLogicValues,
...
void
SimulationInfo::accept(BaseVisitor & inoutVisitor) throw(Error) {
    try
    {
 ...
 */
void 
SimulationInfo::addLogicValue(
        const LogicValueSharedPtr & inLogicValue) throw(Error) {
    if( !inLogicValue )
    {
        return;
    }
 ...
 */
LogicValueSharedPtr 
SimulationInfo::findLogicValue(const std::string & inName) throw() {
    if( inName.empty() )
    {
        Error e( eMessageIdErrorEmptyItemName,
            __FUNCTION__, __FILE__, __LINE__ );
 ...
 */
void
SimulationInfo::removeLogicValue(const std::string &inName) throw(Error) {
    if( inName.empty() )
    {
        Error e( eMessageIdErrorEmptyItemName,
            __FUNCTION__, __FILE__, __LINE__ );
...
SimulationInfo::SimulationInfo()
    ...
    SelfReferencing<SimulationInfo>(),
    UserDataContainer(),
    mLogicValueSymTab() {
}
