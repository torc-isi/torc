...
BOOST_AUTO_TEST_CASE(bitstream_virtexe) {
	...
	boost::filesystem::path generatedPath = regressionPath / "VirtexEUnitTest.generated.bit";
...
//	std::ostream& operator<< (std::ostream& os, const VirtexE::FrameAddress& rhs);
//	std::ostream& operator<< (std::ostream& os, const VirtexE::FrameAddress& rhs) {
...
//	uint32_t frameRowCount = (rowCount / 20) >> 1;
	...
	for(uint32_t i = 0; i < VirtexE::eFarBlockTypeCount; i++) {
		VirtexE::EFarBlockType blockType = VirtexE::EFarBlockType(i);
...
if(i == 2) break;
	...
	/*Need to remove top/bottom & row functionality for VirtexE

	typedef std::map<uint32_t, VirtexE::FrameAddress> FrameIndexToAddress;
	typedef std::map<VirtexE::FrameAddress, uint32_t> FrameAddressToIndex;
			...
			for(uint32_t farRow = 0; farRow < farRowCount; farRow++) {
				...
				for(TileCol col; col < colCount; col++) {
					...
					for(uint32_t farMinor = 0; farMinor < width; farMinor++) {
						VirtexE::FrameAddress far(VirtexE::EFarTopBottom(half), blockType, farRow, 
							farMajor, farMinor);
...
*/
	...
	for(int half = 0; half < 2; half++) {
		for(uint32_t row = 0; row < 2; row++) {
			...
			ColumnMaxFrame maxFrames[VirtexE::eFarBlockTypeCount];
			VirtexE::const_iterator p = bitstream.begin();
			VirtexE::const_iterator e = bitstream.end();
			uint32_t header = VirtexEPacket::makeHeader(VirtexEPacket::ePacketType1, 
				VirtexEPacket::eOpcodeWrite, VirtexE::eRegisterLOUT, 1);
			while(p < e) {
				const VirtexEPacket& packet = *p++;
