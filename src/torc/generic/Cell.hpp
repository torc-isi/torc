 ...
 *
 * The Cell class is used to implement an EDIF cell object. It contains the different views of a
 * cell. A cell can be concrete or extern (black-box).
 *
 * @note  Currently parser performs semantic checks for GENERIC type only. Programattic creation
 * may be achieved. However, a warning may be emitted when cell type is set to TIE or RIPPER.
 */
class Cell : public Commentable, public Extern, public Nameable, public ParentedObject<Library>,
	public PropertyContainer, public Renamable, public Visitable, public SelfReferencing<Cell>,
...
public:
	 ...
	 */
	enum Type {
		eTypeTie = 0, eTypeRipper, eTypeGeneric
	};
	...
	public:
		 ...
		 **/
		virtual CellSharedPtr newCellPtr(const std::string& inName,
			const LibrarySharedPtr& inLibraryPtr, const Cell::Type& inCellType = Cell::eTypeGeneric,
	 ...
	 */
	inline void getViews(std::vector<ViewSharedPtr>& outViews) const;
	 ...
	 *
	 */
	template <typename _Action> inline void applyOnAllViews(const _Action& action) throw (Error);
 ...
 * @return Type of cell
 */
inline const Cell::Type Cell::getType() const {
	return mType;
}
 ...
 */
inline void Cell::getViews(std::vector<ViewSharedPtr>& outViews) const {
	mViewSymTab.getValues(outViews);
}

template <typename _Action> inline void Cell::applyOnAllViews(const _Action& action) throw (Error) {
	try {
