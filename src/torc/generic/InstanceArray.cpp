...
InstanceArraySharedPtr InstanceArray::Factory::newInstanceArrayPtr(const std::string& inName,
	const ViewSharedPtr& inViewPtr, const ViewSharedPtr& inMaster, const size_t& inSize,
	const ChildFactorySharedPtr& inFactory, const std::string& inOriginalName) throw (Error) {
	try {
...
InstanceArraySharedPtr InstanceArray::Factory::newInstanceArrayPtr(const std::string& inName,
	const ViewSharedPtr& inViewPtr, const ViewSharedPtr& inMaster,
...
void InstanceArray::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
...
void InstanceArray::setParent(const ViewSharedPtr& inParent) {
	ParentedObject<View>::setParent(inParent);
	BaseVectorType::List children;
	getCreatedChildren(children);
	std::for_each(children.begin(), children.end(),
		boost::bind(boost::mem_fn(&Instance::setParent), _1, getParent()));
}
...
void InstanceArray::addPortReference(const PortReferenceSharedPtr& inPortRef) throw (Error) {
	//TBD::ERROR UNSUPPORTED
}

PortReferenceSharedPtr InstanceArray::findPortReference(const std::string& inName) throw (Error) {
	//TBD::ERROR UNSUPPORTED
	return PortReferenceSharedPtr();
}

void InstanceArray::removePortReference(const std::string& inName) throw (Error) {
	//TBD::ERROR UNSUPPORTED
}

void InstanceArray::getPortReferences(std::vector<PortReferenceSharedPtr>& outPortRefs) const {
	return;
}

void InstanceArray::setPortReferences(const std::vector<PortReferenceSharedPtr>& inSource)
	throw (Error) {
	//TBD::ERROR UNSUPPORTED
}

void InstanceArray::onChildCreate(const boost::shared_ptr<BaseVectorType::ChildType>&
	inCreatedChild) const throw (Error) {
	inCreatedChild->setName(getName());
}

InstanceArray::InstanceArray() : Instance(), Vector<Instance, InstanceArrayMember,
	InstanceArrayMember::Factory, false>() {}

InstanceArray::~InstanceArray() throw () {}

#ifdef GENOM_SERIALIZATION
template <class Archive> void InstanceArray::serialize(Archive& ar, unsigned int) {
	ar & boost::serialization::base_object<Instance>(*this);
	ar & boost::serialization::base_object< BaseVectorType >(*this);
}

//TO SATISFY THE LINKER
template void InstanceArray::serialize<boost::archive::binary_iarchive>(
	boost::archive::binary_iarchive& ar, const unsigned int);

template void InstanceArray::serialize<boost::archive::binary_oarchive>(
