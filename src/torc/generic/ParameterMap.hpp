...
#ifndef TORC_GENERIC_PARAMETERMAP_HPP
#define TORC_GENERIC_PARAMETERMAP_HPP
...
/**
 * Stores Name-Parameter mappings. This map can be used to get and set values for different
 * parameters.
 */
class ParameterMap {
public:
	 ...
	 */
	void set(const ParameterContext& inContext, const std::string& inName,
	 ...
	 */
	void getAllParameters(const ParameterContext& inContext,
		std::map<std::string, ParameterSharedPtr>& outParams) const;

	/**
	 * Get all parameter name-value-pairs that have been overridden in this context. For a view
	 * context this will return an empty map.
	 * @param[in] inContext the context for which the param is valid
	 * @param[out] outParams Map to be populated
	 */
	void getOverriddenParameters(const ParameterContext& inContext,
		std::map<std::string, ParameterSharedPtr>& outParams) const;
	 ...
	 *
	 */
	template <typename _Action> inline void applyOnAllParameters(const ParameterContext& inContext,
	 ...
	 *
	 */
	template <typename _Action> inline void applyOnOverriddenParameters(
		const ParameterContext& inContext, const _Action& action) throw (Error);

	ParameterContext getNewContext();

	void registerContext(const ParameterContext& inContext,
		const ParameterContext& inParentContext = ParameterContext()) throw (Error);
...
private:
	struct ParamData {
		...
		ParamData() : mParams(), mParentContext() {
		}
	};
...
template <typename _Action> inline void ParameterMap::applyOnAllParameters(
	const ParameterContext& inContext, const _Action& action) throw (Error) {
	if(!inContext) {
		return;
	}
	try {
		...
		if(!data) {
			...
			applyOnAllParameters(data->mParentContext, action);
		}
	} catch(Error& e) {
...
template <typename _Action> inline void ParameterMap::applyOnOverriddenParameters(
	const ParameterContext& inContext, const _Action& action) throw (Error) {
	if(!inContext) {
		return;
	}
...
#endif // TORC_GENERIC_PARAMETERMAP_HPP
