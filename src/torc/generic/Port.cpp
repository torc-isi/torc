...
Port::Port() : Composite<Port>(), Commentable(), Connectable(), Extern(), Nameable(),
	PropertyContainer(), Renamable(), Visitable(), ParentedObject<View>(), UserDataContainer(),
	mDirection(ePortDirectionUndefined), mAttributes() {}
 ...
 */
void Port::setDirection(const EPortDirection& inSource) {
	mDirection = inSource;
}
 ...
 */
void Port::setAttributes(const PortAttributesSharedPtr& inSource) {
	mAttributes = inSource;
}

#ifdef GENOM_SERIALIZATION
template <class Archive> void Port::serialize(Archive& ar, unsigned int) {
	...
	ar & boost::serialization::base_object<Composite<Port> >(*this);
	ar & mDirection;
	ar & mAttributes;
...
template void Port::serialize<boost::archive::binary_iarchive>(
	boost::archive::binary_iarchive& ar, const unsigned int);

template void Port::serialize<boost::archive::binary_oarchive>(
