...
#ifndef TORC_GENERIC_PORTREFERENCE_HPP
#define TORC_GENERIC_PORTREFERENCE_HPP
 ...
 *
 * The PortReference class is used to provide an interface for port usage of an instantiated cell.
 * In other words, it more or less models the (portRef ...) keyword in EDIF. However, unlike a
 * simple portRef, this can have properties that can be assigned to a portRef using
 * (portInstance ...) syntax. This interface can be used to refer to all types of port
 * references(scalar, vector, vector-bit and bundle).
 */
class PortReference : public Connectable, public PropertyContainer, public Visitable,
	virtual public Composite<PortReference>, public ParentedObject<Instance> {
...
public:
	 ...
	 */
	inline PortSharedPtr getMaster() const;
	 ...
	 */
	inline const PortAttributesSharedPtr getAttributes() const;
 ...
 * @return The master port.
 */
inline PortSharedPtr PortReference::getMaster() const {
	return mMaster;
}
 ...
 * @return Pointer to PortAttributes object.
 */
inline const PortAttributesSharedPtr PortReference::getAttributes() const {
	return mAttributes;
}
...
#endif // TORC_GENERIC_PORTREFERENCE_HPP
