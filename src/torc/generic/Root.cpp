...
/**
 * Add a library to the list of libraries. If an empty pointer is supplied, it returns without
 * doing anything.
 *
 ...
 */
void Root::addLibrary(const LibrarySharedPtr& inLibrary) throw (Error) {
	if(!inLibrary) {
		return;
	}
...
#ifdef GENOM_SERIALIZATION
	std::list< std::string >::iterator it = std::find(mDumpedLibraries.begin(),
...
/**
 * Remove the specified library from the list of libraries. If an empty pointer is passed, it
 ...
 */
void Root::removeLibrary(const std::string& inName) throw (Error) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
 ...
 */
void Root::getLibraries(std::vector<LibrarySharedPtr>& outLibraries) {
#ifdef GENOM_SERIALIZATION
	restoreAllLibraries();
#endif //GENOM_SERIALIZATION
 ...
 */
LibrarySharedPtr Root::findLibrary(const std::string& inName) {
 ...
 */
void Root::addDesign(const DesignSharedPtr& inDesign) throw (Error) {
	if(!inDesign) {
		return;
	}
 ...
 */
void Root::removeDesign(const std::string& inName) throw (Error) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
 ...
 */
void Root::getDesigns(std::vector<DesignSharedPtr>& outDesigns) {
 ...
 */
DesignSharedPtr Root::findDesign(const std::string& inName) {
 ...
 */
RootSharedPtr Root::Factory::newRootPtr(const std::string& inName, const EdifLevel& inEdifLevel,
...
void Root::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
 ...
 */
void Root::setVersion(const EdifVersion& inSource) {
	mVersion = inSource;
}
 ...
 */
void Root::setLevel(const EdifLevel& inSource) {
	mLevel = inSource;
}

#ifdef GENOM_SERIALIZATION

void Root::setDumpRestoreData(const DumpRestoreData& inDumpRestoreData) {
	mDumpRestoreData = inDumpRestoreData;
}

void Root::handleNewDumpRestoreData(const DumpRestoreData& inDumpRestoreData) throw(Error) {
	mDumpRestoreData = inDumpRestoreData;
...
#endif //GENOM_SERIALIZATION
Root::Root() : Commentable(), Nameable(), Renamable(), Visitable(), SelfReferencing<Root>(),
	UserDataContainer(), StatusContainer(), mLevel(), mLibraries(), mDesignSymTab(), mVersion()
#ifdef GENOM_SERIALIZATION
, mDumpRestoreData(), mDumpedLibraries()
#endif //GENOM_SERIALIZATION
{}
...
#ifdef GENOM_SERIALIZATION
template <class Archive> void Root::load(Archive& ar, unsigned int) {
	...
	ar & boost::serialization::base_object<SelfReferencing<Root> >(*this);
	ar & mLevel;
	ar & mVersion;
	//ar & mDumpRestoreData;
	ar & mDumpedLibraries;
	//We do not retrieve libraries now
	//We retrieve when required
}

template <class Archive> void Root::save(Archive& ar, unsigned int) const {
...
LibrarySharedPtr Root::restoreSingleLibrary(const std::string& inName) throw(Error) {
	LibrarySharedPtr library;
	std::list< std::string >::iterator lib = find(mDumpedLibraries.begin(),
		mDumpedLibraries.end(), inName);
...
void Root::restoreAllLibraries() throw(Error) {
	try {
		std::vector<std::string> libs;
		libs.insert(libs.end(), mDumpedLibraries.begin(), mDumpedLibraries.end());
...
template void Root::load<boost::archive::binary_iarchive>(boost::archive::binary_iarchive& ar,
	const unsigned int);

template void Root::save<boost::archive::binary_oarchive>(boost::archive::binary_oarchive& ar,
	const unsigned int) const;

void dump(const RootSharedPtr& inRoot) throw(Error) {
	if(!inRoot) {
		//TBD::ERROR
	}
	try {
...
void dump(const RootSharedPtr& inRoot, const DumpRestoreData& inData ) throw(Error) {
	if(!inRoot) {
		//TBD::ERROR
	}
	...
	std::ofstream dout(fileName.c_str(), std::ios::out | std::ios::binary);
	if(!dout) {
		//TBD::ERROR
	}
...
RootSharedPtr restore(const std::string& inName, const DumpRestoreData& inData) throw(Error) {
	RootSharedPtr root;
	if(inName.empty()) {
		//TBD::ERROR
	}
	...
	std::ifstream din(fileName.c_str(), std::ios::out | std::ios::binary);
	if(!din) {
		//TBD::ERROR
	}
