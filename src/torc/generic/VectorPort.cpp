 ...
 **/
VectorPortSharedPtr VectorPort::Factory::newVectorPortPtr(const std::string& inName,
	const EPortDirection& inDirection, const ViewSharedPtr& inViewPtr, const size_t& inSize,
	const PortBundleSharedPtr& inParentCollection, const ChildFactorySharedPtr& inFactory,
	const std::string& inOriginalName) throw (Error) {
	try {
		std::vector < size_t > limits;
		limits.push_back(inSize);
		return newVectorPortPtr(inName, inDirection, inViewPtr, limits, inParentCollection,
			inFactory, inOriginalName);
	} catch(Error& e) {
...
VectorPortSharedPtr VectorPort::Factory::newVectorPortPtr(const std::string& inName,
	...
	const std::vector<size_t>& inLimits, const PortBundleSharedPtr& inParentCollection,
	...
	try {
		...
		newVectorPort->constructChildren(inFactory,
			inLimits);
...
void VectorPort::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
...
void VectorPort::setParent(const ViewSharedPtr& inParent) {
	ParentedObject<View>::setParent(inParent);
	BaseVectorType::List children;
	getCreatedChildren(children);
	std::for_each(children.begin(), children.end(),
		boost::bind(boost::mem_fn(&Port::setParent), _1, getParent()));
}
...
void VectorPort::disconnect(const Connectable::Connection& inConnection) throw (Error) {
...
void VectorPort::getConnectedNets(std::vector<NetSharedPtr>& outNets,
...
#ifdef GENOM_SERIALIZATION
template <class Archive> void VectorPort::serialize(Archive& ar, unsigned int) {
	ar & boost::serialization::base_object<Port>(*this);
	ar & boost::serialization::base_object<BaseVectorType>(*this);
}

//TO SATISFY THE LINKER
template void VectorPort::serialize<boost::archive::binary_iarchive>(
	boost::archive::binary_iarchive& ar, const unsigned int);

template void VectorPort::serialize<boost::archive::binary_oarchive>(
