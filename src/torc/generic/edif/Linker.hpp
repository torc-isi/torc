...
#ifndef TORC_GENERIC_EDIF_LINKER_HPP
#define TORC_GENERIC_EDIF_LINKER_HPP
 ...
 * 
 * The Linker class represents a repository of unresolved instances, that is instances that are yet
 * to to be linked to concrete views. When an instance is created by the parser, it tries to find a
 * concrete view using the name specification present in the EDIF file. If no such references were
 * found, the instance is attached to a Extern view, that is a view that is only a place holder. As
 * newer cells are discovered by the parser, the unresolved instances are linked to the concrete
 * views and removed from the linker.
 * 
...
public:
	...
	public:
		...
		NameSpec(const std::string& inLibraryName, const std::string& inCellName,
		...
		inline bool operator <(const NameSpec& inRhs) const;

		inline std::string getLibraryName() const;

		void setLibraryName(const std::string& inValue);

		inline std::string getCellName() const;

		void setCellName(const std::string& inValue);

		inline std::string getViewName() const;
	...
	public:
		inline const ViewSharedPtr getExternView() const;

		void setExternView(const ViewSharedPtr& inValue);

		void addInstance(const InstanceSharedPtr& inInstance);

		inline const std::vector<InstanceSharedPtr>& getInstances() const;

		void setInstances(const std::vector<InstanceSharedPtr>& value);

		template <typename _Tp> void applyActionOnInstances(const _Tp& action) {
			std::vector<InstanceSharedPtr>::iterator first = mInstances.begin();
			std::vector<InstanceSharedPtr>::iterator last = mInstances.end();
...
public:
	...
	/**
	 * Add an instance to the list of unresolved instances for a given name specification. If a new
	 * UnresolvedInstances object is to be created, this method will also call the setExternView()
	 * method to update the externView, with the view set for this object.
	 *
	 * @param[in] inNameSpec name specification for a view.
	 * @param[in] inInstance instance to be added.
	 *
	 * @exception Error The master for this instance is not an extern or no extern is set for this
	 * view.
	 */
...
inline bool Linker::NameSpec::operator <(const Linker::NameSpec& inRhs) const {
	return mLibraryName < inRhs.mLibraryName
		|| (mLibraryName == inRhs.mLibraryName && mCellName < inRhs.mCellName)
		|| (mLibraryName == inRhs.mLibraryName && mCellName == inRhs.mCellName
			&& mViewName < inRhs.mViewName);
}

inline std::string Linker::NameSpec::getLibraryName() const {
	return mLibraryName;
}

inline std::string Linker::NameSpec::getCellName() const {
	return mCellName;
}

inline std::string Linker::NameSpec::getViewName() const {
	return mViewName;
}

inline const ViewSharedPtr Linker::UnresolvedInstances::getExternView() const {
	return mExternView;
}

inline const std::vector<InstanceSharedPtr>& Linker::UnresolvedInstances::getInstances() const {
	return mInstances;
}
...
#endif // TORC_GENERIC_EDIF_LINKER_HPP
