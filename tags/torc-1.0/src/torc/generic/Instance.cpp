...
#ifdef GENOM_SERIALIZATION
BOOST_IS_ABSTRACT(torc::generic::Composite<torc::generic::Instance>)
#endif //GENOM_SERIALIZATION
#ifdef GENOM_SERIALIZATION
namespace {}
#endif //GENOM_SERIALIZATION
...
class PortMapper {
public:
	void operator()(const torc::generic::PortReferenceSharedPtr& pRef) const
		throw (torc::generic::Error) {
		torc::generic::PortSharedPtr port = mMaster->findPort(pRef->getName());
		if(!port || port->getCompositionType() != pRef->getCompositionType()
	...
	PortMapper(const torc::generic::ViewSharedPtr& inMaster) : mMaster(inMaster) {
	}
...
ParameterContext Instance::getParameterContext() const {
	return mMyContext;
}

ParameterMapSharedPtr Instance::getParameters() const throw (Error) {
	if(!getMaster()) {
		return ParameterMapSharedPtr();
	}
	ParameterMapSharedPtr params = mMaster->getParameters();
	if(!params->isContextRegistered(mMyContext)) {
		params->registerContext(mMyContext, mMaster->getParameterContext());
	}
	return params;
}
 ...
 */
void Instance::bindToMasterView(const ViewSharedPtr& inMaster, bool inMapPortReferences)
	throw (Error) {
	if(!mMaster) {
	...
	if(oldMaster) {
		ParameterMapSharedPtr oldParams = oldMaster->getParameters();
	...
	if(!myParams.empty()) {
		try {
			...
			for(std::map<std::string, ParameterSharedPtr>::iterator it = myParams.begin();
				it != myParams.end(); ++it) {
				paramMap->set(mMyContext, (*it).first, (*it).second);
			}
		} catch(Error& e) {
			e.setCurrentLocation(__FUNCTION__, __FILE__, __LINE__);
			throw;
...
void Instance::setMaster(const ViewSharedPtr& inMaster) {
	mMaster = inMaster;
}
 ...
 */
void Instance::addPortReference(const PortReferenceSharedPtr& inPortRef) throw (Error) {
 ...
 */
PortReferenceSharedPtr Instance::findPortReference(const std::string& inName) throw (Error) {
 ...
 */
NetReferenceSharedPtr Instance::findNetReference(const std::string& inName) throw (Error) {
 ...
 */
void Instance::removePortReference(const std::string& inName) throw (Error) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
 ...
 */
void Instance::getPortReferences(std::vector<PortReferenceSharedPtr>& outPortRefs) const {
 ...
 */
void Instance::setPortReferences(const std::vector<PortReferenceSharedPtr>& inSource) throw (Error) {
...
void Instance::setDesignator(const std::string& inSource) {
 ...
 */
void Instance::setTiming(const TimingSharedPtr& inSource) {
	mTiming = inSource;
}

Instance::Instance() :
	Composite<Instance>(), Commentable(), Nameable(), PropertyContainer(), Renamable(), Visitable(), ParentedObject<
...
#ifdef GENOM_SERIALIZATION
template <class Archive> void Instance::load(Archive& ar, unsigned int) {
	ar & boost::serialization::base_object < Composite<Instance> > (*this);
	...
	mPortReferences.applyOnAll(
		boost::bind(boost::mem_fn(&PortReference::setParent), _1, getSharedThis()));
}

template <class Archive> void Instance::save(Archive& ar, unsigned int) const {
	ar & boost::serialization::base_object < Composite<Instance> > (*this);
	ar & boost::serialization::base_object < Commentable > (*this);
	ar & boost::serialization::base_object < Nameable > (*this);
	ar & boost::serialization::base_object < PropertyContainer > (*this);
	...
	mMasterData->mLibrary = mMaster->getParent()->getParent()->getName();
	mMasterData->mCell = mMaster->getParent()->getName();
	mMasterData->mView = mMaster->getName();
	//We save only our overridden params
	getParameters()->getOverriddenParameters(getParameterContext(), mMasterData->mParams);
	ar & mMasterData;
}

//TO SATISFY THE LINKER
template void Instance::save<boost::archive::binary_oarchive>(boost::archive::binary_oarchive& ar,
	const unsigned int) const;

template void Instance::load<boost::archive::binary_iarchive>(boost::archive::binary_iarchive& ar,
	const unsigned int);

void Instance::restoreMaster() throw (Error) {
	try {
		if(!mMasterData) {
			//TBD::ERROR
		}
		...
		LibrarySharedPtr targetLib = root->findLibrary(mMasterData->mLibrary);
		if(!targetLib) {
			//TBD::ERROR
		}
		CellSharedPtr targetCell = targetLib->findCell(mMasterData->mCell);
		if(!targetCell) {
			//TBD::ERROR
		}
		ViewSharedPtr targetView = targetCell->findView(mMasterData->mView);
		if(!targetView) {
			//TBD::ERROR
		}
		bindToMasterView(targetView);
		//Restore params
		std::map<std::string, ParameterSharedPtr>::iterator it = mMasterData->mParams.begin();
		std::map<std::string, ParameterSharedPtr>::iterator end = mMasterData->mParams.end();
		for(; it != end; ++it) {
			getParameters()->set(mMyContext, (*it).first, (*it).second);
		}
		delete mMasterData;
		mMasterData = NULL;
	} catch(Error& e) {
