...
public:
	...
	void openLogFile(const std::string& inFileName);
...
void Logger::write(const char *fmt, va_list ap) {
	vfprintf(mStream, fmt, ap);
	return;
}

void Logger::openLogFile(const std::string& inFileName) {
	if(!inFileName.empty()) {
