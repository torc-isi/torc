 ...
 */
PermutableSharedPtr Permutable::Factory::newPermutablePtr(const std::vector<PortSharedPtr>& inPorts,
	const ViewSharedPtr& inViewPtr, const PermutableSharedPtr& inParentPermutable) throw (Error) {
	try {
 ...
 */
void Permutable::setPorts(const std::vector<PortSharedPtr>& inSource) throw (Error) {
 ...
 */
void Permutable::addPort(const PortSharedPtr& inPort) throw (Error) {
	if(!inPort) {
		return;
	}
 ...
 */
void Permutable::setChildren(const std::vector<PermutableSharedPtr>& inSource) throw (Error) {
 ...
 * @exception Error Could not add permutable because pointer to the permutable does not exist
 */
bool Permutable::addChildPermutable(const PermutableSharedPtr& inPermutable) throw (Error) {
	if(!inPermutable) {
...
void Permutable::setIsNonPermutable(const bool& value) {
	mIsNonPermutable = value;
}

void Permutable::accept(BaseVisitor& visitor) throw (Error) {
	try {
 ...
 */
size_t Permutable::getSize() const {
	size_t size = 0;
	size_t pSize = 0;
	size_t cSize = 0;
	...
	std::vector<PermutableSharedPtr>::iterator permutableIt = outPermutables.begin();
 ...
 */
void Permutable::setPermutableType(const PermutableType& inSource) {
	mPermutableType = inSource;
}

Permutable::Permutable() : Visitable(), SelfReferencing<Permutable>(), mPorts(), mChildren(),
	mIsNonPermutable(false), mPermutableType() {}
