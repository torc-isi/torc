 ...
 */
void PortElement::setType(const ElementType& inSource) {
	mType = inSource;
}

PortElement::PortElement() : mType(eElementTypePort), mPort(), mPortReference() {}

PortElement::PortElement(const PortSharedPtr& inPort) :
	mType(eElementTypePort), mPort(inPort), mPortReference() {}

PortElement::PortElement(const PortReferenceSharedPtr& inPortRef) :
	mType(eElementTypePortReference), mPort(), mPortReference(inPortRef) {}

PortElement::PortElement(const PortElement& inRhs) :
	mType(inRhs.mType), mPort(inRhs.mPort), mPortReference(inRhs.mPortReference) {}

PortElement& PortElement::operator =(const PortElement& inRhs) {
	if(this == &inRhs) {
		mType = inRhs.mType;
		mPort = inRhs.mPort;
