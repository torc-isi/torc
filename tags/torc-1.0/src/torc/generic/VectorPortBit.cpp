...
void VectorPortBit::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
...
Connectable::Connection VectorPortBit::connect(const NetSharedPtr& inNet) throw (Error) {
	if(!inNet) {
...
void VectorPortBit::disconnect(const Connection& inConnection) throw (Error) {
...
VectorPortBit::VectorPortBit() : Port(), VectorBit<Port>() {}
