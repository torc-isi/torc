...
void VectorPortBitReference::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
...
Connectable::Connection VectorPortBitReference::connect(const NetSharedPtr& inNet) throw (Error) {
	if(!inNet) {
...
void VectorPortBitReference::disconnect(const Connection& inConnection) throw (Error) {
...
void VectorPortBitReference::getConnectedNets(std::vector<NetSharedPtr>& outNets,
	...
	if(!inSkipChildConnections) {
		...
		//parent
		Composite<PortReference>::Pointer parent = getParentCollection();
		if(!parent) {
			Error e(eMessageIdErrorPointerToItemDoesNotExist, __FUNCTION__, __FILE__, __LINE__);
...
VectorPortBitReference::VectorPortBitReference() :
	PortReference(), VectorBit<PortReference>() {}

VectorPortBitReference::~VectorPortBitReference() throw () {}

#ifdef GENOM_SERIALIZATION
template <class Archive> void VectorPortBitReference::serialize(Archive& ar, unsigned int) {
	ar & boost::serialization::base_object<PortReference>(*this);
	ar & boost::serialization::base_object<VectorBit<PortReference> >(*this);
}

//TO SATISFY THE LINKER
template void VectorPortBitReference::serialize<boost::archive::binary_iarchive>(
	boost::archive::binary_iarchive& ar, const unsigned int);

template void VectorPortBitReference::serialize<boost::archive::binary_oarchive>(
