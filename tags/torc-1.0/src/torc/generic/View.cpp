...
public:
	void operator()(const InstanceSharedPtr& inInst) const throw(Error) {
		try {
	...
	RestoredInstanceUpdater(const ViewSharedPtr& inView) : mView(inView) {}
private:
	ViewSharedPtr mView;
};
#endif //GENOM_SERIALIZATION
 ...
 **/
ViewSharedPtr View::Factory::newViewPtr(const std::string& inName, const CellSharedPtr& inCellPtr,
	const View::Type& inViewType, const std::string& inOriginalName) throw (Error) {
	try {
...
void View::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
...
ParameterMapSharedPtr View::getParameters() throw (Error) {
 ...
 */
void View::setType(const View::Type& inSource) {
	mType = inSource;
}
 ...
 */
void View::setInstances(const std::vector<InstanceSharedPtr>& inSource) throw (Error) {
 ...
 */
void View::addInstance(const InstanceSharedPtr& inInstance) throw (Error) {
	if(!inInstance) {
 ...
 */
InstanceSharedPtr View::findInstance(const std::string& inName) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
...
/**
 * Remove the specified instance from the list of cells. If an empty pointer is passed, it returns
 ...
 */
void View::removeInstance(const std::string& inName) throw (Error) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
		e.saveContextData("Instance name", inName);
		throw e;
	...
	if(instance) {
		...
		for_each(refs.begin(), refs.end(),
			boost::bind(boost::mem_fn(&Connectable::disconnect), _1));
 ...
 */
void View::setNets(const std::vector<NetSharedPtr>& inSource) throw (Error) {
	std::vector<NetSharedPtr>::const_iterator net = inSource.begin();
	std::vector<NetSharedPtr>::const_iterator end = inSource.end();
	for(; net != end; ++net) {
		try {
			addNet(*net);
		} catch(Error& e) {
 ...
 */
void View::addNet(const NetSharedPtr& inNet) throw (Error) {
	if(!inNet) {
		return;
	}
 ...
 */
NetSharedPtr View::findNet(const std::string& inName) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
 ...
 */
void View::removeNet(const std::string& inName) throw (Error) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
 ...
 */
void View::setPorts(const std::vector<PortSharedPtr>& inSource) throw (Error) {
 ...
 */
void View::addPort(const PortSharedPtr& inPort) throw (Error) {
	if(!inPort) {
		return;
	}
 ...
 */
PortSharedPtr View::findPort(const std::string& inName) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
...
/**
 * Remove the specified port from the list of ports. If an empty pointer is passed, it returns
 * without doing anything
 *
 ...
 */
void View::removePort(const std::string& inName) throw (Error) {
	if(inName.empty()) {
		Error e(eMessageIdErrorEmptyItemName, __FUNCTION__, __FILE__, __LINE__);
		e.saveContextData("Port name", inName);
		throw e;
...
void View::setNonNetlistViewData(const std::string& inData) {
	mNonNetlistViewData = inData;
}

/**
 * Set the vector of permutables to this view. It will lead to a linear traversal on the list. So
 ...
 */
void View::setPermutables(const std::vector<PermutableSharedPtr>& inSource) throw (Error) {
 ...
 * @exception Error Could not add permutable because pointer to the permutable does not exist
 */
bool View::addPermutable(const PermutableSharedPtr& inPermutable) throw (Error) {
	if(!inPermutable) {
		Error e(eMessageIdErrorPointerToItemDoesNotExist, __FUNCTION__, __FILE__, __LINE__);
 ...
 */
void View::setInterfaceJoinedInfos(const std::vector<InterfaceJoinedInfoSharedPtr> & inSource)
	throw (Error) {
 ...
 * @exception Error Could not add joined info because pointer to the joined info does not exist
 */
bool View::addInterfaceJoinedInfo(const InterfaceJoinedInfoSharedPtr& inJoinedInfo) throw (Error) {
	if(!inJoinedInfo) {
		Error e(eMessageIdErrorPointerToItemDoesNotExist, __FUNCTION__, __FILE__, __LINE__);
 ...
 */
void View::setInterfaceAttributes(const InterfaceAttributesSharedPtr& inSource) {
 ...
 */
void View::setSimulate(const SimulateSharedPtr& inSource) {
	mSimulate = inSource;
}
 ...
 */
void View::setTiming(const TimingSharedPtr& inSource) {
	mTiming = inSource;
}

View::View() : Commentable(), Extern(), Nameable(), ParentedObject<Cell>(), PropertyContainer(),
	Renamable(), SelfReferencing<View>(), Visitable(), UserDataContainer(), StatusContainer(),
	mParameters(new ParameterMap()), mMyContext(mParameters->getNewContext()), mInstanceSymTab(),
	mNetSymTab(), mPortSymTab(), mType(eTypeNetlist), mPermutables(), mInterfaceJoinedInfos(),
	mAttributes(), mSimulate(), mTiming() {}

View::~View() throw () {
	if(!getName().empty()) {
		log("View %s is being destroyed.\n", getName().c_str());
	}
	mNetSymTab.applyOnAll(boost::bind(boost::mem_fn(&Net::disconnect), _1));
	mNetSymTab.clear();
	mPortSymTab.clear();
...
#ifdef GENOM_SERIALIZATION
template <class Archive> void View::load(Archive& ar, unsigned int) {
	...
	ar & boost::serialization::base_object<SelfReferencing<View> >(*this);
...
template <class Archive> void View::save(Archive& ar, unsigned int) const {
	ar & boost::serialization::base_object<Extern>(*this);
	ar & boost::serialization::base_object<Nameable>(*this);
	ar & boost::serialization::base_object<PropertyContainer>(*this);
	ar & boost::serialization::base_object<Renamable>(*this);
...
void View::restoreActions() throw(Error) {
	try {
		mInstanceSymTab.applyOnAll(RestoredInstanceUpdater(getSharedThis()));
		mInstanceSymTab.applyOnAll(boost::bind(boost::mem_fn(&Net::setParent), _1,
			getSharedThis()));
		mInstanceSymTab.applyOnAll(boost::bind(boost::mem_fn(&Port::setParent), _1,
			getSharedThis()));
	} catch(Error& e) {
		e.setCurrentLocation(__FUNCTION__, __FILE__, __LINE__);
		throw;
...
template void View::load<boost::archive::binary_iarchive>(boost::archive::binary_iarchive& ar,
	const unsigned int);

template void View::save<boost::archive::binary_oarchive>(boost::archive::binary_oarchive& ar,
