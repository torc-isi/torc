...
#ifndef TORC_GENERIC_CONNECTABLE_HPP
#define TORC_GENERIC_CONNECTABLE_HPP
 ...
 *
 * The Connectable class can act as a base for objects that can connect to a Net. This interface
 * provides event functionality for connection and disconnection. It also provides polymorphic
 * methods to implement connectivity beween different components in a design.
 */
class Connectable {
#ifdef GENOM_SERIALIZATION
...
public:
	 ...
	 * A signal to indicate that a new connection has been made
	 */
	inline Connected& signalNetConnected();

	/**
	 * A signal to indicate that a new connection has been made
	 */
	inline Disconnected& signalNetDisconnected();
	 ...
	 */
	virtual void getConnectedNets(std::vector<NetSharedPtr>& outNets, bool inSkipChildConnections =
		false) const throw (Error);
	 ...
	 */
	virtual Connection connect(const NetSharedPtr& inNet) throw (Error) = 0;
	 ...
	 */
	virtual void disconnect(const Connection& inConnection) throw (Error) = 0;
 ...
 * A signal to indicate that a new connection has been made
 */
inline Connectable::Connected& Connectable::signalNetConnected() {
 ...
 * A signal to indicate that a new connection has been made
 */
inline Connectable::Disconnected& Connectable::signalNetDisconnected() {
...
#endif // GENOM_SERIALIZATION
#endif // TORC_GENERIC_CONNECTABLE_HPP
