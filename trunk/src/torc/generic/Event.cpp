 ...
 */
EventSharedPtr Event::Factory::newEventPtr(const Event::Type& inEventType,
	const std::list<PortSharedPtr>& inPorts,
	const std::list<PortReferenceSharedPtr>& inPortReferences,
	const std::list<NetSharedPtr>& inNets, const LogicElementSharedPtr& inTransition,
	const PortListSharedPtr& inPortList, const Value& inOffsetTime) throw (Error) {
	try {
...
void Event::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
 ...
 */
void Event::setType(const Type& inSource) {
	mType = inSource;
}
 ...
 */
void Event::addPort(const PortSharedPtr& inPort) {
 ...
 */
void Event::addPortReference(const PortReferenceSharedPtr& inPortRef) {
...
void Event::setPortList(const PortListSharedPtr& inPortList) {
 ...
 */
void Event::addNet(const NetSharedPtr& inNet) {
 ...
 */
void Event::setTransition(const LogicElementSharedPtr& inSource) {
 ...
 */
void Event::setOffsetTime(const Value& value) {
	mOffsetTime = value;
}

Event::Event() : Visitable(), SelfReferencing<Event>(), mType(), mPortElements(), mPortList(),
	mNets(), mTransition(), mOffsetTime() {}
