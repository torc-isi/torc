 ...
 **/
PortListAliasSharedPtr PortListAlias::Factory::newPortListAliasPtr(const std::string& inName,
	const PortListSharedPtr& inPortList, const SimulateSharedPtr& inSimulate) throw (Error) {
	try {
...
PortListAlias::PortListAlias() : Nameable(), SelfReferencing<PortListAlias>(), mPortList() {}

void PortListAlias::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
...
void PortListAlias::setPortList(const PortListSharedPtr& inPortList) {
