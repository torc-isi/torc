...
#ifndef TORC_GENERIC_PROPERTYCONTAINER_HPP
#define TORC_GENERIC_PROPERTYCONTAINER_HPP
 ...
 *
 * The PropertyContainer interface is generalized by classes tat need to hold one or more
 * properties. A property is identified by name in the container.
 */
...
public:
	 ...
	 */
	inline void getProperties(std::map<std::string, PropertySharedPtr>& outProperties) const;
	 ...
	 *
	 */
	template <typename _Action> inline void applyOnAllProperties(const _Action& action)
		throw (Error);
 ...
 */
inline void PropertyContainer::getProperties(
	std::map<std::string, PropertySharedPtr>& outProperties) const {
	mProperties.getValueMap(outProperties);
	return;
}

template <typename _Action> inline void PropertyContainer::applyOnAllProperties(
...
#endif // TORC_GENERIC_PROPERTYCONTAINER_HPP
