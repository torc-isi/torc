 ...
 *
 * The Root class represents and EDIF hierarchy. All parsed EDIF files contribute to the growth of
 * the object tree rooted here. At the top level, it provides access to the libraries present in
 * the design.
 *
 * @note In case the tree has been programatically created, clients need to decompile this tree, a
 * name has to be given to the tree, otherwise an exception will be generated. This name will
 * become the design name of the decompiled output file. For parser generated trees, the design
 * name of the last file parsed becomes the name of the root. This can obviously be changed by the
 * client, by calling the set_name() method.
 */
class Root : public Commentable, public Nameable, public Renamable, public Visitable,
	public SelfReferencing<Root>, public UserDataContainer, public StatusContainer {
...
public:
	...
	public:
		 ...
		 */
		virtual RootSharedPtr newRootPtr(const std::string& inName, const EdifLevel& inEdifLevel =
			eEdifLevel0, const std::string& inOriginalName = std::string()) throw (Error);
	};
	 ...
	 * @note If some libraries are yet to be restored, this method is not thread safe. For
	 ...
	 * restoration of all libraries under the root.
	 *
	 */
	template <typename _Action> inline void applyOnAllLibraries(const _Action& action) throw (Error);
	 ...
	 *
	 */
	template <typename _Action> inline void applyOnAllDesigns(const _Action& action) throw (Error);
	 ...
	 */
	inline const EdifVersion getVersion() const;
...
#ifdef GENOM_SERIALIZATION
	inline const DumpRestoreData& getDumpRestoreData() const;

	void setDumpRestoreData(const DumpRestoreData& inDumpRestoreData);
	void handleNewDumpRestoreData(const DumpRestoreData& inDumpRestoreData) throw(Error);
#endif //GENOM_SERIALIZATION
	virtual ~Root() throw ();
...
private:
	...
	LibrarySharedPtr restoreSingleLibrary(const std::string& inName) throw(Error);

	void restoreAllLibraries() throw(Error);
 ...
 *
 */
template <typename _Action> inline void Root::applyOnAllLibraries(const _Action& action)
	throw (Error) {
	try {
#ifdef GENOM_SERIALIZATION
		restoreAllLibraries();
 ...
 *
 */
template <typename _Action> inline void Root::applyOnAllDesigns(const _Action& action)
	throw (Error) {
	try {
 ...
 * @return Version of edif
 */
inline const EdifVersion Root::getVersion() const {
	return mVersion;
}
 ...
 * @return The EDIF level for this file
 */
inline const EdifLevel Root::getLevel() const {
	return mLevel;
}

#ifdef GENOM_SERIALIZATION
inline const DumpRestoreData&
Root::getDumpRestoreData() const {
	return mDumpRestoreData;
}

void dump(const RootSharedPtr& inRoot) throw(Error);

void dump( const RootSharedPtr& inRoot,
	const DumpRestoreData& inData ) throw(Error);

RootSharedPtr restore( const std::string& inName,
	const DumpRestoreData& inData ) throw(Error);
