 ...
 */
TimingSharedPtr Timing::Factory::newTimingPtr(const Derivation& inDerivation,
	const std::list<PathDelaySharedPtr>& inPathDelays,
	const std::list<ForbiddenEventSharedPtr>& inForbiddentEvents, const ViewSharedPtr& inView,
	const InterfaceAttributesSharedPtr& inInterfaceAttributes, const InstanceSharedPtr& inInstance)
		throw (Error) {
	try {
...
void Timing::accept(BaseVisitor& inoutVisitor) throw (Error) {
	try {
 ...
 */
void Timing::setDerivation(const Derivation& value) {
	mDerivation = value;
}
 ...
 * @param[in] inPathDelays List of path delays of this timing to be appended to
 */
void Timing::setPathDelays(const std::list<PathDelaySharedPtr>& inPathDelays) {
 ...
 */
void Timing::setForbiddentEvents(const std::list<ForbiddenEventSharedPtr>& inForbiddentEvents) {
 ...
 */
void Timing::addPathDelay(const PathDelaySharedPtr& inSource) {
	try {
 ...
 */
void Timing::addForbiddenEvent(const ForbiddenEventSharedPtr& inSource) {
	try {
...
Timing::Timing() : Visitable(), SelfReferencing<Timing>(), Commentable(), UserDataContainer(),
	mDerivation(), mPathDelays(), mForbiddentEvents() {}
