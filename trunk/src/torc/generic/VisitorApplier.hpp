...
template <typename _Tp> class VisitorApplier {
public:
	VisitorApplier(_Tp& inVisitor) :
		mVisitor(inVisitor) {
	}

	template <typename _Up> void operator()(const boost::shared_ptr<_Up>& inVisited) const
		throw (Error) {
		try {
...
#endif // TORC_GENERIC_VISITORAPPLIER_HPP
