...
#ifndef TORC_GENERIC_EDIF_PARSERHELPERS_HPP
#define TORC_GENERIC_EDIF_PARSERHELPERS_HPP
...
struct ViewRefData {
	...
	ViewRefData() : mViewName(NULL), mCellName(NULL), mLibraryName(NULL) {
	}
...
struct CellRefData {
	...
	CellRefData() : mCellName(NULL), mLibraryName(NULL) {
	}
...
struct LogicRefData {
	...
	LogicRefData() : mLogicName(NULL), mLibraryName(NULL) {
	}
...
struct PortRefData {
	...
	PortRefData() : mParentPort(NULL), mPortName(NULL), mInstanceName(NULL), mView(NULL) {
	}
...
struct NetRefData {
	...
	NetRefData() : mParentNet(NULL), mNetName(NULL), mInstanceName(NULL), mView(NULL) {
	}
...
struct PortInstData {
	...
	PortInstData() : mRefData(NULL), mAttribute(NULL) {
	}
...
struct PortListData {
	...
	PortListData() : mPortRefs() {
	}

	~PortListData() {
		for(std::vector<PortRefData*>::iterator it = mPortRefs.begin(); it != mPortRefs.end();
			++it) {
			delete *it;
		}
	}
...
struct PairData {
	...
	PairData(int32_t inX = 0, int32_t inY = 0) : mX(inX), mY(inY) {
	}
};
...
struct ParamAssignInfo {
	...
	ParamAssignInfo(NameData* inNameData = NULL, ValueData* inValueData = NULL)
		: mNameData(inNameData), mValueData(inValueData) {
	}
	~ParamAssignInfo() {
		delete mNameData;
		delete mValueData;
...
struct InstanceInfo {
	...
	InstanceInfo() : mViewRefData(NULL), mParamAssign(), mPortInst(), mUserData(), mComment(),
		mDesignator(), mTiming() {
	}

	~InstanceInfo() {
		delete mViewRefData;
		for(std::vector<ParamAssignInfo*>::iterator it = mParamAssign.begin();
			it != mParamAssign.end(); it++) {
			delete (*it);
		}
		for(std::vector<PortInstData*>::iterator it = mPortInst.begin(); it != mPortInst.end();
			it++) {
			delete (*it);
		}
		mUserData.clear();
...
struct LogicListData {
	...
	LogicListData() : mNameDataList() {
	}

	~LogicListData() {
		for(std::list<NameData*>::iterator it = mNameDataList.begin(); it != mNameDataList.end();
			++it) {
			delete *it;
		}
	}
...
#endif // TORC_GENERIC_EDIF_PARSERHELPERS_HPP
